...
class PlanningPage(QWidget):
    ...
    def _load_events(self):
        ...
        events = Event.get_all(user=self.user, department=dept, include_completed=show_completed)
        
        # Фильтрация по дате
        selected_date = self.date_input.date().toPyDate()
        filtered_events = [e for e in events if e.event_date == selected_date] if selected_date else events
    ...
    def _show_context_menu(self, pos):
        ...
        # Переключить статус
        if event:
            status_action = menu.addAction("✅ Выполнено" if not event.is_completed else "↩️ Не выполнено")
