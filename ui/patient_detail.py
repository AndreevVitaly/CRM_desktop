...
from models.db_models import (User, Patient, Encounter, Note, Prescription, 
                              Diagnosis, TreatmentPlanItem, PatientInteraction,
...
class PatientDetailDialog(QDialog):
    ...
    def _create_header(self) -> QFrame:
        ...
        name_label.setStyleSheet(f"font-size: {FONTS['size_xlarge']}pt; font-weight: bold;")
    ...
    def _create_info_tab(self) -> QWidget:
        ...
        fields = [
            ...
            ("Тип пациента:", "Взрослый" if self.patient.patient_type == "adult" else "Детский"),
            ("Отделение:", self.patient.department_display),
            ("Лечащий врач:", self.patient.doctor.full_name if self.patient.doctor else "Не назначен"),
        ]
        ...
        if self.patient.facility:
            facility_label = QLabel(f"📍 {self.patient.facility.name} ({self.patient.facility.type_display})")
    ...
    def _create_encounters_tab(self) -> QWidget:
        ...
        if self.user.role in (User.ROLE_ADMIN, User.ROLE_REGISTRAR, User.ROLE_LEAD, User.ROLE_DOCTOR, User.ROLE_NURSE):
    ...
    def _load_encounters(self):
        ...
        for encounter in encounters:
            ...
            self.encounters_table.setItem(row, 2, QTableWidgetItem(encounter.reason or "—"))
            
            status_item = QTableWidgetItem(encounter.status_display)
            status_item.setForeground(Qt.GlobalColor.darkGreen if encounter.status == Encounter.STATUS_FINISHED else Qt.GlobalColor.darkBlue)
    ...
    def _load_plan_items(self):
        ...
        for item in plan_items:
            ...
            status_item.setForeground(Qt.GlobalColor.darkGreen if item.is_completed else Qt.GlobalColor.darkBlue)
    ...
    def _load_log(self):
        ...
        for interaction in interactions:
            ...
            self.log_table.setItem(row, 3, QTableWidgetItem(interaction.description or "—"))
    ...
    def _open_encounter(self, index):
        ...
        from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel,
                                     QFrame, QPushButton, QTextEdit, QFormLayout)
        from PyQt6.QtCore import Qt
        from datetime import datetime
        
        dialog = QDialog(self)
        dialog.setWindowTitle(f"Визит от {encounter.started_at.strftime('%d.%m.%Y %H:%M')}")
        ...
        info_layout.addRow("Дата:", QLabel(encounter.started_at.strftime("%d.%m.%Y %H:%M")))
        info_layout.addRow("Врач:", QLabel(encounter.doctor.full_name if encounter.doctor else "—"))
        info_layout.addRow("Причина:", QLabel(encounter.reason or "—"))
        
        status_label = QLabel(encounter.status_display)
        status_label.setStyleSheet("font-weight: bold; color: {};"
                                   .format(colors['success'] if encounter.status == Encounter.STATUS_FINISHED else colors['warning']))
        ...
        if notes:
            for note in notes:
                ...
                note_header = QLabel(f"<b>{note.author.full_name}</b> • {note.created_at.strftime('%d.%m.%Y %H:%M') if note.created_at else ''}")
                note_header.setStyleSheet("color: {}; font-size: {}pt;"
                                         .format(colors['text_muted'], FONTS['size_small']))
        ...
        if prescriptions:
            for rx in prescriptions:
                ...
                rx_details = QLabel(f"Частота: {rx.frequency} • Длительность: {rx.duration_days} дн.")
    ...
    def _toggle_plan_item(self):
        ...
        if row < len(plan_items):
            ...
            item.toggle()
            self._load_plan_items()
            self._log_interaction("plan_item_toggle", f"{'Выполнен' if item.is_completed else 'Отменён'} пункт: {item.event}")
