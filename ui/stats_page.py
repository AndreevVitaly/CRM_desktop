...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                             QFrame, QComboBox, QDateEdit, QTableWidget,
...
class StatsPage(QWidget):
    ...
    def _get_kpi_data(self) -> list:
        ...
        visits = Encounter.get_all(
            user=self.user if self.user.role not in (User.ROLE_ADMIN, User.ROLE_REGISTRAR) else None,
        ...
        if self.user.role == User.ROLE_ADMIN:
            ...
            dept_patients = [p for p in patients if p.department == self.user.department]
            return [
                ("👥", "Пациентов отделения", str(len(dept_patients)), ""),
                ("👨", "Взрослых", str(len([p for p in dept_patients if p.patient_type == "adult"])), ""),
                ("👶", "Детей", str(len([p for p in dept_patients if p.patient_type == "child"])), ""),
    ...
    def _create_kpi_card(self, icon: str, title: str, value: str, subtitle: str) -> QFrame:
        """KPI карточка"""
        colors = get_colors()
